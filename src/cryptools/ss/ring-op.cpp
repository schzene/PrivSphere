...
template <class T>
std::vector<T> FixArray::get_native_type() const {
...
std::ostream& operator<<(std::ostream& os, FixArray& other) {
    ...
    for (int i = 0; i < other.size; i++) {
        ...
        os << dbl_other[i] << " int=" << data_ << " (" << tmp_data.substr(64 - other.ell, 64) << ")\t";
    }
    os << "]";
    return os;
}

template vector<uint32_t> FixArray::get_native_type() const;
template vector<int32_t> FixArray::get_native_type() const;
template vector<uint64_t> FixArray::get_native_type() const;
template vector<int64_t> FixArray::get_native_type() const;
template vector<float> FixArray::get_native_type() const;
template vector<double> FixArray::get_native_type() const;

/**
 * arrayop function IMPlement
 * from EzPC
 * 2024.1122
 */
...
FixArray FixOp::if_else(const BoolArray& cond, const FixArray& x, const FixArray& y) {
...
FixArray FixOp::mul(const FixArray& x, const FixArray& y, int ell, uint8_t* msb_x, uint8_t* msb_y) {
    ...
    if (x.party == PUBLIC || y.party == PUBLIC) {
        ...
        mult->hadamard_product(x.size, x.data, y.data, ret.data, x.ell, y.ell, ell, x.signed_, y.signed_,
                               MultMode::None, msb_x, msb_y);
    }
    return ret;
}
...
FixArray FixOp::left_shift(const FixArray& x, const FixArray& s, int ell, int bound, uint8_t* msb_x) {
    ...
    if (party == ALICE) {
        ...
        aux->lookup_table<uint64_t>(nullptr, s.data, pow2_s.data, x.size, m, pow2_s_ell);
    }
...
FixArray FixOp::right_shift(const FixArray& x, const FixArray& s, int bound, uint8_t* msb_x) {
    assert(x.party != PUBLIC && s.party != PUBLIC);
    assert(x.size == s.size);
    ...
    if (party == ALICE) {
        ...
        aux->lookup_table<uint64_t>(nullptr, s.data, pow2_neg_s.data, x.size, m, bound + 2);
    }
    BoolArray all_0 = bool_op->input(ALICE, x.size, uint8_t(0));
...
FixArray FixOp::LUT(const vector<uint64_t>& spec_vec, const FixArray& x, bool signed_, int l_out, int s_out, int l_in) {
    assert(x.party != PUBLIC);
    ...
    if (party == ALICE) {
        ...
        aux->lookup_table<uint64_t>(nullptr, x_red.data, ret.data, x.size, l_in, l_out);
    }
    return ret;
}
...
FixArray FixOp::sirnn_exp(const FixArray& x, int l_y, int s_y, int digit_size) {
    assert(x.party != PUBLIC);
    ...
    for (int i = 1; i < num_digits; i *= 2) {
        for (int j = 0; j < num_digits and j + i < num_digits; j += 2 * i) {
            digits_exp[j] = this->mul(digits_exp[j + i], digits_exp[j], 2 * s_y + 2, all_0.data, all_0.data);
...
FixArray FixOp::max_bolt(const vector<FixArray>& x) {
    int N       = x.size();
    int n       = x[0].size;
    int party   = x[0].party;
...
FixArray FixOp::tree_sum_bolt(const vector<FixArray>& x) {
    int N       = x.size();
    int n       = x[0].size;
    int party   = x[0].party;
    int signed_ = x[0].signed_;
...
FixArray FixOp::sirnn_div(const FixArray& nm, const FixArray& dn, int l_out, int s_out, bool normalized_dn) {
    if (!normalized_dn)
        assert(dn.signed_ == false);
...
FixArray FixOp::sirnn_div_batch(const FixArray& nm, const FixArray& dn, int batch_dn_size, int l_out, int s_out,
                                bool normalized_dn) {
    if (!normalized_dn)
        assert(dn.signed_ == false);
    assert(nm.party != PUBLIC && dn.party != PUBLIC);
...
FixArray FixOp::sirnn_sigmoid(const FixArray& x, int l_y, int s_y) {
    assert(x.party != PUBLIC);
    assert(x.signed_ == true);
    ...
    FixArray exp_neg_x = fix->sirnn_exp(neg_x, s_y + 2, s_y);
    ...
    FixArray inv_dn = fix->sirnn_div(one_dn, dn, s_y + 2, s_y, true);
...
FixArray FixOp::sirnn_tanh(const FixArray& x, int l_y, int s_y) {
    assert(x.party != PUBLIC);
    assert(x.signed_ == true);
    ...
    FixArray exp_neg_2x = fix->sirnn_exp(neg_2x, s_y + 2, s_y);
    ...
    FixArray tanh_neg_x = fix->sirnn_div(nm, dn, s_y + 2, s_y, true);
...
FixArray FixOp::sirnn_sqrt(const FixArray& x, int l_y, int s_y, bool recp_sqrt) {
    assert(x.party != PUBLIC);
    // "Not Implement"
    return x;
}

FixArray FixOp::abs_bolt(const FixArray& x) {
    BoolArray msb_x = MSB(x);
    FixArray neg_x  = mul(x, -1);
    return if_else(msb_x, neg_x, x);
}

void FixOp::send_fix_array(const FixArray& x) {
    iopack->io->send_data(&x.party, sizeof(int));
    if (x.party == nisl::PUBLIC) {
        iopack->io->send_data(&x.size, sizeof(int));
        iopack->io->send_data(x.data, sizeof(uint64_t) * x.size);
        iopack->io->send_data(&x.signed_, sizeof(bool));
        iopack->io->send_data(&x.ell, sizeof(int));
        iopack->io->send_data(&x.s, sizeof(int));
    }
    iopack->io->flush();
}

void FixOp::recv_fix_array(FixArray& x) {
    iopack->io->recv_data(&x.party, sizeof(int));
    if (x.party == nisl::PUBLIC) {
        iopack->io->recv_data(&x.size, sizeof(int));
        if (x.data == nullptr) {
            x.data = new uint64_t[x.size];
        }
        iopack->io->recv_data(x.data, sizeof(uint64_t) * x.size);
        iopack->io->recv_data(&x.signed_, sizeof(bool));
        iopack->io->recv_data(&x.ell, sizeof(int));
        iopack->io->recv_data(&x.s, sizeof(int));
    }
    iopack->io->flush();
}

FixArray FixOp::local_truncate(const FixArray& x, int scale) {
...
FixArray FixOp::local_div(const FixArray& nm, const FixArray& dn) {
    assert(nm.size == dn.size);
    vector<double> dn_double = dn.get_native_type<double>();
    FixArray div_dn(dn.party, dn.size, dn.signed_, dn.ell, dn.s);
    for (size_t i = 0; i < dn.size; i++) {
        div_dn.data[i] = nisl::neg_mod(static_cast<int64_t>(1. / dn_double[i] * (1ULL << dn.ell)), (1ULL << dn.ell));
    }
    FixArray ret = mul(nm, div_dn, nm.ell);
    return ret;
}