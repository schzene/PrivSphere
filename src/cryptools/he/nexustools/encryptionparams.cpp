...
namespace nexus {
namespace seal_bs {
    const seal::parms_id_type parms_id_zero = seal::util::HashFunction::hash_zero_block;
    ...
    void EncryptionParameters::save_members(ostream &stream) const
    {
        ...
        try
        {
            ...
            {
                mod.save(stream, seal::compr_mode_type::none);
            }

            // Only BFV and BGV uses plain_modulus but save it in any case for simplicity
            plain_modulus_.save(stream, seal::compr_mode_type::none);
        }
    ...
    void EncryptionParameters::load_members(istream &stream, SEAL_MAYBE_UNUSED seal::SEALVersion version)
    {
        // Throw exceptions on std::ios_base::badbit and std::ios_base::failbit
        auto old_except_mask = stream.exceptions();
        try
