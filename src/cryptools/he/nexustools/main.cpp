...
vector<string> TEST_TARGETS = {"MatMul", "Argmax", "GELU", "LayerNorm", "SoftMax", "SgnFunc"};
...
int main() {
  ...
  */
    if (TEST_TARGET == TEST_TARGETS[2]) {
        ...
        cout << "Mean Absolute Error: "
             << ckks_evaluator.calculateMAE(gelu_calibration, cipher_output, poly_modulus_degree / 2) << endl;
  ...
  */
    if (TEST_TARGET == TEST_TARGETS[3]) {
        ...
        cout << "[LayerNorm] 16 x 768 takes: " << duration_cast<milliseconds>(end - start).count() << " milliseconds"
             << endl;
        cout << "Mean Absolute Error: " << ckks_evaluator.calculateMAE(layernorm_calibration, cipher_output, 768)
             << endl;
...
void argmax_test() {
    ...
    Bootstrapper bootstrapper(loge, logn, logN - 1, total_level, scale, boundary_K, deg, scale_factor, inverse_deg,
                              context, keygen, encoder, encryptor, decryptor, evaluator, relin_keys,
                              bootstrapping_keys);
    ...
    cout << "[Argmax] 32768 takes: " << duration_cast<milliseconds>(end - start).count() << " milliseconds" << endl;
    cout << "Mean Absolute Error: " << ckks_evaluator.calculateMAE(argmax_calibration, cipher_output, argmax_input_size)
         << endl;
}
...
void MM_test() {
    ...
    std::vector<std::vector<double>> matrix_4096x768 =
        mme.readMatrix("../data/input/matrixmul_input_m_128_n_768_k_64_batch_128.txt", 4096, 768);
    std::vector<std::vector<double>> matrix_768x64 =
        mme.readMatrix("../data/input/matrix_input_n_768_k_64.txt", 768, 64);
    ...
    std::vector<std::vector<double>> matrix_4096x64 =
        mme.readMatrix("../data/calibration/matrix_output_m_128_k_64_batch_128.txt", 4096, 64);
...
void Sngfun_test() {
    ...
    CKKSEvaluator ckks_evaluator(context, encryptor, decryptor, encoder, evaluator, scale, relin_keys, galois_keys);
    SgnFuncEvaluator sgn_evaluator(ckks_evaluator);

    size_t slot_count    = encoder.slot_count();
    vector<double> input = {0.23342, -0.3434, 0.62323, 0.834545, -0.7872};
    ...
    ckks_evaluator.encryptor->encrypt(plain_input, cipher_input);
    cout << "start sgn \n";
    cipher_output = ckks_evaluator.sgn_eval(cipher_input, 2, 2, 1);
    cout << "end sgn \n";
    Plaintext out_sgn;
    ckks_evaluator.decryptor->decrypt(cipher_output, out_sgn);
    vector<double> sgn_res;
    ckks_evaluator.encoder->decode(out_sgn, sgn_res);
    for (size_t i = 0; i < 10; i++) {
        cout << "Sgn function output: " << sgn_res[i] << endl;
    }
}