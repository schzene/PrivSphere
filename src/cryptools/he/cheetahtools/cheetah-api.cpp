...
#include "constants.h"  // ALICE & BOB
#include "net_io_channel.h"
#include "shape_inference.h"
#include "tensor_encoder.h"

template <class CtType>
void send_ciphertext(nisl::NetIO* io, const CtType& ct) {
    std::stringstream os;
    uint64_t ct_size;
    ct.save(os);
    ct_size       = os.tellp();
...
static void send_encrypted_vector(nisl::NetIO* io, const EncVecCtType& ct_vec) {
...
static void recv_encrypted_vector(nisl::NetIO* io, const seal::SEALContext& context,
                                  std::vector<seal::Ciphertext>& ct_vec, bool is_truncated = false);
static void recv_ciphertext(nisl::NetIO* io, const seal::SEALContext& context, seal::Ciphertext& ct,
...
uint64_t CheetahLinear::io_counter() const {
    return io_ ? io_->counter : 0;
}
...
CheetahLinear::CheetahLinear(int party, nisl::NetIO* io, uint64_t base_mod, size_t nthreads)
    ...
    context_ = std::make_shared<SEALContext>(seal_parms, true, sec_level_type::tc128);
...
void SummaryTensor(Tensor<double> const& t, std::string tag) {
    ...
    std::cout << tag << " shape " << t.shape() << " values in [" << mn << "," << mx << "]\n";
}

bool CheetahLinear::verify(const Tensor<uint64_t>& in_tensor_share, const std::vector<Tensor<uint64_t>>& filters,
                           const ConvMeta& meta, const Tensor<uint64_t>& out_tensor_share, int nbit_precision) const {
    ...
    if (party_ == nisl::BOB) {
        io_->send_data(in_tensor_share.data(), sizeof(uint64_t) * in_tensor_share.NumElements());
        io_->send_data(out_tensor_share.data(), sizeof(uint64_t) * out_tensor_share.NumElements());
    }
    else {
        std::vector<uint64_t> in_tensor_raw(ishape.num_elements());
        std::vector<uint64_t> out_tensor_raw(oshape.num_elements());
        io_->recv_data(in_tensor_raw.data(), sizeof(uint64_t) * in_tensor_raw.size());
        io_->recv_data(out_tensor_raw.data(), sizeof(uint64_t) * out_tensor_raw.size());
        ...
        in_tensor.tensor()  = in_tensor.tensor().unaryExpr([this](uint64_t v) { return reduce(v); });
        out_tensor.tensor() = out_tensor.tensor().unaryExpr([this](uint64_t v) { return reduce(v); });
        ...
        f64_in.tensor() = in_tensor.tensor().unaryExpr([&](uint64_t v) { return cast_to_double(v, nbit_precision); });
...
void CheetahLinear::fc(const Tensor<uint64_t>& input_vector, const Tensor<uint64_t>& weight_matrix, const FCMeta& meta,
    ...
    if (party_ == nisl::ALICE && !weight_matrix.shape().IsSameSize(meta.weight_shape)) {
    ...
    if (party_ == nisl::BOB) {
        {
            ...
            if (code != Code::OK) {
                throw std::runtime_error("CheetahLinear::fc encryptInputVector [" + CodeMessage(code) + "]");
            }
        ...
        if (code != Code::OK) {
            throw std::runtime_error("CheetahLinear::fc decryptToVector [" + CodeMessage(code) + "]");
        }
    }
    else {
        std::vector<std::vector<seal::Plaintext>> encoded_matrix;
        code = impl.encodeWeightMatrix(weight_matrix, meta, encoded_matrix, nthreads);
        if (code != Code::OK) {
            throw std::runtime_error("CheetahLinear::fc encodeWeightMatrix error [" + CodeMessage(code) + "]");
        }
        ...
        if (meta.is_shared_input) {
            ...
            if (code != Code::OK) {
                throw std::runtime_error("CheetahLinear::fc encodeInputVector error [" + CodeMessage(code) + "]");
            }
        }
        ...
        auto code =
            impl.matVecMul(encoded_matrix, vec_share0, vec_share1, meta, out_vec_share0, out_vec_share, nthreads);
        if (code != Code::OK) {
            throw std::runtime_error("CheetahLinear::fc matmul2D error [" + CodeMessage(code) + "]");
        }
        send_encrypted_vector(io_, out_vec_share0);
    }
}

void CheetahLinear::conv2d(const Tensor<uint64_t>& in_tensor, const std::vector<Tensor<uint64_t>>& filters,
    ...
    if (meta.n_filters != filters.size()) {
        throw std::invalid_argument("CheetahLinear::conv2d meta.n_filters mismatch");
    }
    ...
    if (party_ == nisl::BOB) {
        {
            ...
            if (code != Code::OK) {
                throw std::runtime_error("CheetahLinear::conv2d encryptImage " + CodeMessage(code));
            }
            send_encrypted_vector(io_, ct_buff);
        }
        ...
        if (code != Code::OK) {
            throw std::runtime_error("CheetahLinear::conv2d decryptToTensor " + CodeMessage(code));
        }
    }
    else {
        ...
        if (code != Code::OK) {
            throw std::runtime_error("CheetahLinear::conv2d ecnodeFilters " + CodeMessage(code));
        }
        ...
        if (meta.is_shared_input) {
            ...
            if (code != Code::OK) {
                throw std::runtime_error("CheetahLinear::conv2d encodeImage " + CodeMessage(code));
            }
        }
        ...
        auto code = impl.conv2DSS(ct_buff, encoded_share, encoded_filters, meta, out_ct, out_tensor, nthreads_);
        if (code != Code::OK) {
            throw std::runtime_error("CheetahLinear::conv2d conv2DSS: " + CodeMessage(code));
        }
        send_encrypted_vector(io_, out_ct);
    }
}

void CheetahLinear::bn(const Tensor<uint64_t>& input_vector, const Tensor<uint64_t>& scale_vector, const BNMeta& meta,
                       Tensor<uint64_t>& out_vector) const {
    if (meta.is_shared_input && !input_vector.shape().IsSameSize(meta.vec_shape)) {
...
void CheetahLinear::bn_direct(const Tensor<uint64_t>& input_tensor, const Tensor<uint64_t>& scale_vector,
                              const BNMeta& meta, Tensor<uint64_t>& out_tensor) const {
    ...
    if (party_ == nisl::BOB) {
        {
            ...
            if (code != Code::OK) {
                throw std::runtime_error("bn_direct encryptVector [" + CodeMessage(code) + "]");
            }
            send_encrypted_vector(io_, ct_buff);
        }
        ...
        if (code != Code::OK) {
            throw std::runtime_error("bn_direct decryptToTensor [" + CodeMessage(code) + "]");
        }
    }
    else {
        if (scale_vector.dims() != 1 || scale_vector.length() != input_tensor.channels()) {
            throw std::runtime_error("bn_direct scale_vector shape mismatch");
        }

        std::vector<seal::Plaintext> encoded_tensor;
        if (meta.is_shared_input) {
            code = bn_impl_.encodeTensor(input_tensor, meta, encoded_tensor, nthreads_);
            if (code != Code::OK) {
                throw std::runtime_error("bn_direct encodeVector [" + CodeMessage(code) + "]");
            }
        }
        ...
        std::vector<seal::Ciphertext> out_ct;
        code = bn_impl_.bn_direct(encrypted_tensor, encoded_tensor, scale_vector, meta, out_ct, out_tensor, nthreads_);
        if (code != Code::OK) {
...
void recv_encrypted_vector(nisl::NetIO* io, const seal::SEALContext& context, std::vector<seal::Ciphertext>& ct_vec,
                           bool is_truncated) {
    uint32_t ncts{0};
...
void recv_ciphertext(nisl::NetIO* io, const seal::SEALContext& context, seal::Ciphertext& ct, bool is_truncated) {
    std::stringstream is;
    uint64_t ct_size;
