...
public:
    ...
    Code setUp(uint64_t target_base_mod, const std::vector<seal::SEALContext>& contexts,
               std::vector<std::optional<seal::SecretKey>> sks, std::vector<std::shared_ptr<seal::PublicKey>> pks);

    Code setUp(uint64_t target_base_mod, const seal::SEALContext& context,
               std::optional<seal::SecretKey> sk = std::nullopt, std::shared_ptr<seal::PublicKey> pk = nullptr);
    ...
    Code encryptVector(const Tensor<uint64_t>& in_vec, const Meta& meta,
                       std::vector<seal::Serializable<seal::Ciphertext>>& out, size_t nthreads = 1) const;

    Code encodeVector(const Tensor<uint64_t>& in_vec, const Meta& meta, std::vector<seal::Plaintext>& out,
                      size_t nthreads = 1) const;

    Code encodeScales(const Tensor<uint64_t>& scales, const Meta& meta, std::vector<seal::Plaintext>& out,
                      size_t nthreads = 1) const;

    Code bn(const std::vector<seal::Ciphertext>& vec_share0, const std::vector<seal::Plaintext>& vec_share1,
            const std::vector<seal::Plaintext>& scales, const Meta& meta, std::vector<seal::Ciphertext>& out_share0,
            Tensor<uint64_t>& out_share1, size_t nthreads = 1) const;

    Code decryptToVector(const std::vector<seal::Ciphertext>& in_vec, const Meta& meta, Tensor<uint64_t>& out_vec,
                         size_t nthreads = 1) const;

    Code encryptTensor(const Tensor<uint64_t>& in_tensor, const Meta& meta,
                       std::vector<seal::Serializable<seal::Ciphertext>>& out, size_t nthreads = 1) const;

    Code encodeTensor(const Tensor<uint64_t>& in_tensor, const Meta& meta, std::vector<seal::Plaintext>& out,
                      size_t nthreads = 1) const;

    Code bn_direct(const std::vector<seal::Ciphertext>& tensor_share0,
                   const std::vector<seal::Plaintext>& tensor_share1, const Tensor<uint64_t>& scales, const Meta& meta,
                   std::vector<seal::Ciphertext>& out_share0, Tensor<uint64_t>& out_share1, size_t nthreads = 1) const;

    Code decryptToTensor(const std::vector<seal::Ciphertext>& in_vec, const Meta& meta, Tensor<uint64_t>& out_tensor,
                         size_t nthreads = 1) const;
...
protected:
    ...
    Code addMaskPrimeField(std::vector<seal::Ciphertext>& ct, Tensor<uint64_t>& mask, const Meta& meta,
                           ThreadPool&) const;
    Code addMaskRing(std::vector<seal::Ciphertext>& ct, Tensor<uint64_t>& mask, const Meta& meta, ThreadPool&) const;
    Code addMask(std::vector<seal::Ciphertext>& ct, Tensor<uint64_t>& mask, const Meta& meta, ThreadPool&) const;
