...
#ifndef NISL_CHEETAH_CHEETAH_API_H_
#define NISL_CHEETAH_CHEETAH_API_H_
...
public:
    ...
    CheetahLinear(int party, nisl::NetIO* io, uint64_t base_mod, size_t nthreads = 1);

    ~CheetahLinear() = default;

    // HomConv
    void conv2d(const Tensor<uint64_t>& in_tensor, const std::vector<Tensor<uint64_t>>& filters, const ConvMeta& meta,
                Tensor<uint64_t>& out_tensor) const;

    // HomFC
    void fc(const Tensor<uint64_t>& input_matrix, const Tensor<uint64_t>& weight_matrix, const FCMeta& meta,
            Tensor<uint64_t>& out_matrix) const;

    // The CrypTFlow2's like BN protocol.
    // Need element-wise multiplication.
    void bn(const Tensor<uint64_t>& input_vector, const Tensor<uint64_t>& scale_vector, const BNMeta& meta,
            Tensor<uint64_t>& out_vector) const;

    // HomBN
    void bn_direct(const Tensor<uint64_t>& input_tensor, const Tensor<uint64_t>& scale_vector, const BNMeta& meta,
                   Tensor<uint64_t>& out_tensor) const;
    ...
    bool verify(const Tensor<uint64_t>& int_tensor, const std::vector<Tensor<uint64_t>>& filters, const ConvMeta& meta,
                const Tensor<uint64_t>& computed_tensor, int nbit_precision) const;
