...
NEXUS_op::NEXUS_op(int party, NetIO* io, unsigned long logN, vector<int> COEFF_MODULI, double _scale)
    ...
    mm_ckks_evaluator =
        new CKKSEvaluator(*context, *encryptor, *decryptor, *encoder, *evaluator, scale, relin_keys, mm_galois_keys);
    bootstrapper =
        new Bootstrapper(loge, logN - 2, logN - 1, total_level, scale, boundary_K, deg, scale_factor, inverse_deg,
                         *context, *keygen, *encoder, *encryptor, *decryptor, *evaluator, relin_keys, mm_galois_keys);
...
void NEXUS_op::gelu(const Data& input, Data& output) {
    ...
    for (size_t i = 0; i < num_feature; i++) {
        for (size_t j = 0; j < dim1; j++) {
            ...
            std::copy(output_vector.begin(), output_vector.begin() + dim2, output.NexusData[i][j].begin());
        }
    }
}
