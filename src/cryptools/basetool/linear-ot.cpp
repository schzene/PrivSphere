    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            if (d == 1) {
                ...
                memcpy(A + (j * m + i) * d, tmpA + (i * n + j) * d, d * sizeof(uint64_t));
            }
        }
    }
    delete[] tmpA;
...
void LinearOT::hadamard_cleartext(int dim, uint64_t* inA, uint64_t* inB, uint64_t* outC) {
    matmul_cleartext(1, dim, 1, inA, inB, outC, false);
}

#ifdef USE_EIGEN
void matmul_cleartext_eigen(int dim1, int dim2, int dim3, uint64_t* inA, uint64_t* inB, uint64_t* outC) {
...
void LinearOT::matmul_cleartext(int dim1, int dim2, int dim3, uint64_t* inA, uint64_t* inB, uint64_t* outC,
                                bool accumulate) {
    if (!accumulate) {
        for (int i = 0; i < dim1; i++) {
            for (int j = 0; j < dim3; j++) {
                for (int k = 0; k < dim2; k++) {
                    outC[dim2 * dim3 * i + dim2 * j + k] = inA[dim2 * i + k] * inB[dim3 * k + j];
                }
            }
        }
        return;
...
void LinearOT::hadamard_cross_terms(int32_t dim, uint64_t* inA, uint64_t* inB, uint64_t* outC, int32_t bwA, int32_t bwB,
                                    int32_t bwC, MultMode mode) {
    matmul_cross_terms(1, dim, 1, inA, inB, outC, bwA, bwB, bwC, false, mode);
}

void LinearOT::hadamard_product(int32_t dim, uint64_t* inA, uint64_t* inB, uint64_t* outC, int32_t bwA, int32_t bwB,
                                int32_t bwC, bool signed_arithmetic, bool signed_B, MultMode mode, uint8_t* msbA,
                                uint8_t* msbB) {
    matrix_multiplication(1, dim, 1, inA, inB, outC, bwA, bwB, bwC, signed_arithmetic, signed_B, false, mode, msbA,
                          msbB);
}
...
void LinearOT::matmul_cross_terms(int32_t dim1, int32_t dim2, int32_t dim3, uint64_t* inA, uint64_t* inB,
                                  uint64_t* outC, int32_t bwA, int32_t bwB, int32_t bwC, bool accumulate,
                                  MultMode mode) {
        ...
        for (int j = i; j < i + batch_size and j < num_ot; j += dim) {
            ...
            for (int k = j; k < j + dim and k < i + batch_size; k++) {
                ...
                for (int h = 0; h < msgs_per_ot; h++) {
                    ...
                    corr[(k - i) * msgs_per_ot + h] = ((elemS << bit_idx) & maskC) >> bit_idx;
                }
...
#pragma omp parallel num_threads(2)
        {
            if (omp_get_thread_num() == 1 && use_reversed_ot) {
                if (party == sci::ALICE) {
                    if (batch_size <= num_ot - i) {
                        otpack->iknp_reversed->recv_batched_cot(ABr, choice, msg_len, batch_size, msgs_per_ot);
                    }
                    else {
                        otpack->iknp_reversed->recv_batched_cot(ABr, choice, msg_len, num_ot - i, msgs_per_ot);
                    }
                }
                else {  // party == sci::BOB
                    if (batch_size <= num_ot - i) {
                        otpack->iknp_reversed->send_batched_cot(ABs, corr, msg_len, batch_size, msgs_per_ot);
                    }
                    else {
                        otpack->iknp_reversed->send_batched_cot(ABs, corr, msg_len, num_ot - i, msgs_per_ot);
                    }
                }
            }
                ...
                if (party == sci::ALICE) {
                    if (batch_size <= num_ot - i) {
                        otpack->iknp_straight->send_batched_cot(ABs, corr, msg_len, batch_size, msgs_per_ot);
                    }
                    else {
                        otpack->iknp_straight->send_batched_cot(ABs, corr, msg_len, num_ot - i, msgs_per_ot);
                    }
                }
                else {  // party == sci::BOB
                    if (batch_size <= num_ot - i) {
                        otpack->iknp_straight->recv_batched_cot(ABr, choice, msg_len, batch_size, msgs_per_ot);
                    }
                    else {
                        otpack->iknp_straight->recv_batched_cot(ABr, choice, msg_len, num_ot - i, msgs_per_ot);
                    }
                }
            }
        }
...
void LinearOT::matmul_multiplexer(int32_t dim1, int32_t dim2, int32_t dim3, uint64_t* inA, uint64_t* inB,
                                  uint64_t* outC, int32_t bwA, int32_t bwB, int32_t bwC, bool accumulate,
                                  MultMode mode) {
    assert(bwA == 1 || bwB == 1);
    assert(bwC < (bwA + bwB));
...
#pragma omp parallel num_threads(2)
    {
        if (omp_get_thread_num() == 1 && use_reversed_ot) {
            if (party == sci::ALICE) {
                otpack->iknp_reversed->recv_batched_got(ABr, choice, dim, bwC, msgs_per_ot);
            }
            else {  // party == sci::BOB
            ...
            if (party == sci::ALICE) {
                ...
                otpack->iknp_straight->recv_batched_got(ABr, choice, dim, bwC, msgs_per_ot);
            }
        }
    }
    for (int h = 0; h < dim2; h++) {
...
void LinearOT::matrix_multiplication(int32_t dim1, int32_t dim2, int32_t dim3, uint64_t* inA, uint64_t* inB,
                                     uint64_t* outC, int32_t bwA, int32_t bwB, int32_t bwC, bool signed_arithmetic,
                                     bool signed_B, bool accumulate, MultMode mode, uint8_t* msbA, uint8_t* msbB) {
    assert(bwC <= 64);
    ...
    if (sender_A) {
        ...
        if (mode == MultMode::Alice_has_B || mode == MultMode::Bob_has_B) {
            for (int i = 0; i < dim2 * dim3; i++) {
                tmpB[i] = ((signed_arithmetic && signed_B) ? signed_val(inB[i], bwB) : inB[i]);
            }
        }
        else {
    ...
    matmul_cross_terms(dim1, dim2, dim3, tmpA, tmpB, cross_terms, bwA, bwB, bwC, accumulate, mode);
    /* print_vec(this, cross_terms, bwC, dim); */

    uint64_t* local_terms = new uint64_t[dim];
    if (party == ALICE && (mode == MultMode::Alice_has_A || mode == MultMode::Alice_has_B)) {
        matmul_cleartext(dim1, dim2, dim3, tmpA, tmpB, local_terms, accumulate);
    }
    else if (party == BOB && (mode == MultMode::Bob_has_A || mode == MultMode::Bob_has_B)) {
        matmul_cleartext(dim1, dim2, dim3, tmpA, tmpB, local_terms, accumulate);
    }
    ...
    if (bwC > bwA) {
        if (mode == MultMode::Alice_has_A || mode == MultMode::Bob_has_A) {
            ...
            matmul_multiplexer(dim1, dim2, dim3, wA64, tmpB, wA_B, 1, bwB, bwC - bwA, accumulate, mode);
            delete[] wA64;
        }
    }
    if (bwC > bwB) {
        if (mode == MultMode::Alice_has_B || mode == MultMode::Bob_has_B) {
            ...
            matmul_multiplexer(dim1, dim2, dim3, tmpA, wB64, wB_A, bwA, 1, bwC - bwB, accumulate, mode);
            delete[] wB64;
        }
    }
    ...
    for (int i = 0; i < dim; i++) {
        tmpC[i] = (local_terms[i] + cross_terms[i] - pow_A * wA_B[i] - pow_B * wB_A[i]) & maskC;
        if (signed_arithmetic) {
            for (int j = 0; j < inner_loop_size; j++) {
                ...
                if (signed_B) {
                    if (party == ALICE) {
                        ...
                        tmpC[i] = (tmpC[i] - pow_A_2 * (tmpB[B_idx] - pow_B * wB[B_idx]) -
                                   pow_B_2 * (tmpA[A_idx] - pow_A * wA[A_idx]) + pow_A_2 * pow_B_2) &
                                  maskC;
                    }
                }
                else {
                    tmpC[i] = (tmpC[i] - pow_A_2 * (tmpB[B_idx] - pow_B * wB[B_idx])) & maskC;
                }
            }
        }
    }
