...
void Truncation::div_pow2(int32_t dim, uint64_t* inA, uint64_t* outB, int32_t shift, int32_t bw, bool signed_arithmetic,
                          uint8_t* msb_x) {
    ...
    uint64_t mask_upper = ((bw - shift - 1) == 64 ? -1 : ((1ULL << (bw - shift - 1)) - 1));
    ...
    this->mill_eq->compare_with_eq(msb_upper, eq_upper, inA_upper, dim, (bw - shift - 1));
    ...
    for (int i = 0; i < dim; i++) {
        outB[i] = ((inA[i] >> shift) + arith_div_correction[i] + arith_wrap_lower[i] -
...
void Truncation::truncate(int32_t dim, uint64_t* inA, uint64_t* outB, int32_t shift, int32_t bw, bool signed_arithmetic,
                          uint8_t* msb_x) {
    if (shift == 0) {
        memcpy(outB, inA, sizeof(uint64_t) * dim);
        return;
    ...
    uint64_t mask_bw    = (bw == 64 ? -1 : ((1ULL << bw) - 1));
    uint64_t mask_shift = (shift == 64 ? -1 : ((1ULL << shift) - 1));
    uint64_t mask_upper = ((bw - shift) == 64 ? -1 : ((1ULL << (bw - shift)) - 1));
    ...
    this->aux->wrap_computation(inA_lower, wrap_lower, dim, shift);
    if (msb_x == nullptr) {
        this->mill_eq->compare_with_eq(wrap_upper, eq_upper, inA_upper, dim, (bw - shift));
        this->aux->AND(wrap_lower, eq_upper, and_upper, dim);
        for (int i = 0; i < dim; i++) {
    ...
    for (int i = 0; i < dim; i++) {
        outB[i] =
            (((inA[i] >> shift) & mask_upper) + arith_wrap_lower[i] - (1ULL << (bw - shift)) * arith_wrap_upper[i]) &
            mask_bw;
    }
...
void Truncation::truncate_red_then_ext(int32_t dim, uint64_t* inA, uint64_t* outB, int32_t shift, int32_t bw,
...
void Truncation::truncate_and_reduce(int32_t dim, uint64_t* inA, uint64_t* outB, int32_t shift, int32_t bw) {
    if (shift == 0) {
        memcpy(outB, inA, sizeof(uint64_t) * dim);
        return;
    }
